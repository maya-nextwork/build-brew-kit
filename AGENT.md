...
== PHOTOS ==
iPhone photos are HEIC, so convert to JPG before importing to Paper: sips -s format jpeg -Z 1600 IN.HEIC --out OUT.jpg (a one-line prep step, not a render pipeline). Warm/dim shots grade cinematic (great for A, G, Build Master story); bright, empty-topped shots suit D. The board applies the grade: one consistent cinematic treatment (see brand/grade.md; for magenta/RGB party lighting use the strong colored-lighting branch there). Do not pre-edit or use a heavy filter. Never the pixelated-photo look.
...
== EXPORT (only after Gate 2, once they've seen and approved the preview) ==
...
Route 2 has one difference, and you should say it plainly rather than let it surprise them: **the house fonts aren't included in the kit**, so posts render with close stand-ins. Layout, grade, colour, crop, and export sizes are all identical, and the result is genuinely postable. Say it in one line and move on; don't turn it into a caveat that makes them think the output is second-rate. If they do have Paper, Route 1 gives sharper type.
